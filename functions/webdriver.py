...
# def screenshot_to_report(context, screenshot_location):
#     """
#     Description:
#         Takes a screenshot of the browser and saves it to the specified
#         location. The screenshot will be named 'screenshot-<timestamp>.jpg'.
#         Note that this method is to be used when run locally. The commented
#         method of the same name above should be used when run via Jenkins.
#     Args:
#         screenshot_location (str): Directory to store the screenshot.
#     """
#     # Ensure screenshot_location has trailing slash
#     if not screenshot_location[-1] == '/':
#         screenshot_location += '/'
#
#     now = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
#     filename = "{}screenshot-{}.png".format(screenshot_location, now)
...
def locate_element_by(context, by_loc, locator):
    ...
    try:
        ...
        element = wait.until(
            lambda browser: getattr(browser, by_loc)(locator))
        context.exc = None
        return element
